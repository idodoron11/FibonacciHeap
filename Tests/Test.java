...
public class Test {
    ...
    static void test7() {
        ...
        for (int i = 2000; i < 2500; i++) {
            ...
            int fiboHeapSize = fibonacciHeap.size();
            if (heapMin != fiboHeapMin || heapSize != fiboHeapSize) {
                bugFound(test);
                return;
            }
            heap.delete(i);
    ...
    static void test22() {
        ...
        ArrayList<FibonacciHeap.HeapNode> nodes = new ArrayList<>();
        // 32767 trees are added. not 32768.
        for (int i = treeSize; i < treeSize * 2; i++) {
            nodes.add(fibonacciHeap.insert(i));
            potential = fibonacciHeap.potential();
        }
        // 1000 trees are added.
        for (int i = 0; i < sizeToDelete; i++) {
            fibonacciHeap.insert(i);
            potential = fibonacciHeap.potential();
        }
        // 1000 trees are deleted.
        for (int i = 0; i < sizeToDelete; i++) {
            fibonacciHeap.deleteMin();
            potential = fibonacciHeap.potential();
        }
